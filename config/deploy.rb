...
role :app, "147.96.240.11"
role :db, "147.96.240.11", primary: true

set :migrate_target,  :current
# set :ssh_options,     { :forward_agent => true }
# set :gateway, 'crowdtshirt@crowdtshirt.com'
set :deploy_to,       "/var/www/sue/sue-web"
...
namespace :deploy do
  ...
  task :restart, :except => { :no_release => true } do
    run "kill -s USR2 `cat #{latest_release}/tmp/pids/unicorn.sue_global_ror.pid`"
  end
  ...
  task :stop, :except => { :no_release => true } do
    run "kill -s QUIT `cat #{latest_release}/tmp/pids/unicorn.sue_global_ror.pid`"
  end
